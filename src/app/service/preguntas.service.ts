...
import { emprendedorPreguntaDTO } from '../model/emprendedorPreguntaDTO';
...
export class PreguntasService {
  ...
  getPreguntaCountByEmprendedor(): Observable<emprendedorPreguntaDTO[]> {
    let token = sessionStorage.getItem("token");
    return this.http.get<emprendedorPreguntaDTO[]>(`${this.url}/preguntas-count`,
    {
      headers: new HttpHeaders().set('Authorization', `Bearer ${token}`).set('Content-Type', 'application/json')
    });
  }
